...
def query_llm_for_analysis(source_code: str,package_name: str) -> str:
    # 使用配置中的 provider 和 model
    client = get_llm_client(LLM_PROVIDER, LLM_MODEL)
    system_prompt = system_prompt_a

    error1_path = LLM_MODEL + "max_retries"
    error2_path = LLM_MODEL + "other_question"
    max_retries = 3
...
def main_pipeline(base_dir: str, results_base_dir: str, max_workers: int = 10):
    ...
    total = len(all_package_folders)
    return total, success_count, 0, fail_count
